(() => {
  ...
  var p = (h, t, e) =>
    t in h ? m(h, t, {enumerable: !0, configurable: !0, writable: !0, value: e}) : (h[t] = e);
  var l = (h, t, e) => (p(h, typeof t != 'symbol' ? t + '' : t, e), e);
  var r = class r extends HTMLElement {
    constructor() {
      super(),
    ...
    static async ready(t, e) {
      if ((e || (e = r.getParents(t)), e.length === 0)) return;
    ...
    replaceTemplates(t) {
      for (let e of t) {
        if (r.getParents(e, this).length > 0) continue;
        let a = e.getAttribute(r.attr.template);
        if (a === 'replace') {
          let i = Array.from(this.childNodes);
          for (let o of i) this.removeChild(o);
    ...
    async hydrate() {
      ...
      for (let a in e) c.map[a] && t.push(c.map[a](e[a], this));
      await Promise.all(t), this.replaceTemplates(this.getTemplates());
      for (let a of r.onReady.values()) await a.call(this, r);
      this.readyResolve(),
        this.setAttribute(r.attr.ready, ''),
        this.querySelectorAll(`[${r.attr.defer}]`).forEach(a => a.removeAttribute(r.attr.defer));
    }
  };
  l(r, 'tagName', 'is-land'),
    l(r, 'prefix', 'is-land--'),
    l(r, 'attr', {template: 'data-island', ready: 'ready', defer: 'defer-hydration'}),
    l(r, 'onceCache', new Map()),
    l(r, 'onReady', new Map()),
    l(r, 'fallback', {
      ':not(is-land,:defined,[defer-hydration])': (t, e, a) => {
        let i = document.createElement(a + e.localName);
        for (let n of e.getAttributeNames()) i.setAttribute(n, e.getAttribute(n));
        let o = e.shadowRoot;
        if (!o) {
        ...
        return (
          o && i.attachShadow({mode: o.mode}).append(...o.childNodes),
          i.append(...e.childNodes),
          e.replaceWith(i),
          t.then(() => {
            i.shadowRoot && e.shadowRoot.append(...i.shadowRoot.childNodes),
              e.append(...i.childNodes),
              i.replaceWith(e);
          })
        );
      }
  ...
  var d = r,
    s = class s {
      ...
      static getConditions(t) {
        let e = {};
        for (let a of Object.keys(s.map)) t.hasAttribute(`on:${a}`) && (e[a] = t.getAttribute(`on:${a}`));
        return e;
      }
      static visible(t, e) {
      ...
      static interaction(t, e) {
        ...
        return (
          t && (a = (t || '').split(',').map(i => i.trim())),
          new Promise(i => {
            function o(n) {
              i();
              for (let f of a) e.removeEventListener(f, o);
            }
            for (let n of a) e.addEventListener(n, o, {once: !0});
          })
        );
      }
      static media(t) {
